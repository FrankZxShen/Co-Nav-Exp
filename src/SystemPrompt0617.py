...
You are a robot that is exploring an indoor environment. You can access a scene image and top-down semantic graph of the current time step of one of the robots.
The right part of the image is the scene image of the current timestep. 
...
Scene object (object detection): format-<Name, Confidence Score, Boundary Coordinates>
...
### F/N
FN_System_Prompt = '''
frontier points (The black dots and corresponding uppercase letters on the left image):
{FRONTIERS_RESULTS}
historical observation points (The green dots and corresponding uppercase letters on the left image)
{HISTORY_NODES}
The arrow : [[Coordinates: {CUR_LOCATION}].
Based on the current top-down semantic map of an indoor environment provided (The left part of the image), you will see various points marked as either 'frontier points' or 'historical observation points'. Frontier points represent unexplored areas that the robot has yet to navigate, while historical observation points signify areas the robot has previously explored or observed. The arrow is your location.

Considering whether you think the current scene is worth exploring, you must decide on the next point to explore.
...
Your choice is 'Frontier'. So you will now explore the frontiers in this indoor environment.
...
Your choice is 'History Nodes'. So you will now re-explore historical nodes in this indoor environment. 

The green dots and corresponding uppercase letters on the left image represent the Frontiers awaiting your exploration: 
{HISTORY_NODES}
where the confidence level following each letter represents the exploration likelihood that this point was recorded. Point with higher confidence have higher exploration priority.
The arrow is your location: [[Coordinates: {CUR_LOCATION}].
...
Your choice must be in "A, B, C, D..." or other 26 capital letters representing historical observation points  **WITHOUT ANY OTHER DESCRIPTION**. You don't need to add punctuation at the end. You only need to answer "A", "B", "C", "D" or other 26 capital letters".
'''
...
def form_prompt_for_FN(Frontier_list, cur_location, History_nodes) -> str:
    ...
    if len(History_nodes) > 0:
        ...
        for hs in History_nodes:
            History.append(f'{chr(65+i)}: [Coordinates: {hs}') 
        His_results = ''
...
def form_prompt_for_DecisionVLM_History(target, cur_location, confidence, History_nodes) -> str:
    ...
    if len(History_nodes) > 0:
        ...
        for i in range(len(History_nodes)):
            History.append(f'{chr(65+i)}: [Coordinates: {History_nodes[i]}; Confidence: {confidence[i]}') 
        His_results = ''
        for i in range(len(History)):
            His_results += History[i]
            His_results += '\n'
...
def Perception_weight_decision26(VLM_Rel: list, VLM_Pred: str) -> dict:
    ...
    assert len(VLM_Rel) == 26, "VLM_Rel must contain weights for 26 decisions (A-Z)."
    
    weights = {chr(65 + i): VLM_Rel[i] for i in range(26)}  # Assign weights to each letter
