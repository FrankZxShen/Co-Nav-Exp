    ...
    while count_episodes < num_episodes:
        ...
        while not env.episode_over:
            ...
            if args.visualize or args.print_images: 
                Visualize0(args, agent[0].episode_n, agent[0].l_step, pose_pred, full_map_pred, 
            ...
            if agent[0].l_step % args.num_local_steps == args.num_local_steps - 1 or agent[0].l_step == 0:
                ...
                if len(full_target_point_map) > 0:
                    ...
                    for j in range(num_agents):
                        ...
                        #### 感知VLM
                        VLM_Perception_Prompt = form_prompt_for_PerceptionVLM(goal_name, agent_objs[f'agent_{j}'], agents_seg_list)
                        ...
                        if len(agent_TargetPointMap[j]) > 0:
                            ...
                            logging.info(f'=====> history_score: {history_score}')
                            Scores = []
                            full_Frontier_list_copy = full_Frontier_list.copy()
                            if FN_PR[0] >= 0.50:
                                ### 退出队列
