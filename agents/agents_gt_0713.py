...
class LLM_Agent_GT(Agent):
    ...
    def _preprocess_obs_rednet(self, obs, semantic, cur_mapping, use_seg=True):
        ...
        # fn = 'VLM_EXP/tmp/VisStep-{}.png'.format(
        #                 self.l_step)
        # cv2.imwrite(fn, semantic)  
        # print(semantic)
        # non_zero_indices = np.nonzero(semantic)
        # print(non_zero_indices[0])
        # np.savetxt('sematiccs.txt', semantic, fmt='%.2f', delimiter=' ')
        # exit(0)

        # red_semantic_pred, semantic_pred = self._get_sem_pred_rednet(
        #     rgb.astype(np.uint8), depth, use_seg=use_seg)

        # print(red_semantic_pred)
        # print(semantic)
        # np.savetxt('rednettts.txt', red_semantic_pred, fmt='%.2f', delimiter=' ')
        # semantic_mask_sofa = np.ones(semantic.shape) * 255
        # for i in range(0,40):
        #     semantic_mask_sofa[semantic==i] = i*5
        # cv2.imwrite("mask_obj" + ".png",semantic_mask_sofa)
        # cv2.imwrite("rgb1" + ".png",rgb.astype(np.uint8))

        if 'objectnav_hm3d' in args.task_config:
            count_goals = 0
            for i in range(len(semantic)):
                for j in range(len(semantic[i])):
                    if hm3d_category[coco_categories[self.goal_id]] in cur_mapping:
                        if cur_mapping[hm3d_category[coco_categories[self.goal_id]]] == semantic[i][j]:
                            # print(cur_mapping[hm3d_category[coco_categories[self.goal_id]]])
                            count_goals += 1
            if count_goals >= 1:
                logging.info("==========> Find GT_Goal!")
                self.Find_Goal = True
            else:
                self.Find_Goal = False
            ...
            # for i in range(len(semantic)):
            #     for j in range(len(semantic[i])):
            #         if object_category[self.goal_id] in cur_mapping:
                        # if cur_mapping[object_category[self.goal_id]] == semantic[i][j]:
                        #     # print(cur_mapping[hm3d_category[coco_categories[self.goal_id]]])
                        #     count_goals += 1
            # print("self.goal_id: ",self.goal_id)
            for i in range(len(semantic)):
                for j in range(len(semantic[i])):
                    if object_category[self.goal_id] in cur_mapping:
                        if cur_mapping[object_category[self.goal_id]] == semantic[i][j]:
