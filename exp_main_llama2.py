...
def Objects_Extract(full_map_pred):
    ...
    for i in range(len(semantic_map)):
        if semantic_map[i, :, :].sum() != 0:
            ...
            # filename = f'/home/szx/project/Co-NavGPT/tmp/{i}.png'
            # cv2.imwrite(filename, se_object_map)
            for cnt in contours:
                if len(cnt) > 30:
...
def main():
    ...
    import utils.llama2_gen as llm_gen
    from utils.llama2_gen import build_llama2
    ...
    model_llm, tokenizer = load_model("/home/szx/data/models/llms/llama-2-7b-hf") ##暂时定义绝对路径
            ...
            if agent[0].l_step % args.num_local_steps == args.num_local_steps - 1 or agent[0].l_step == 0:
                ...
                if len(target_point_map) > 0:
                    ...
                    while retries > 0:  
                        ...
                        # print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%")
                        print("Llama2 Prompt: ",LLM_outputs)
                        # exit(0)
                        goal_frontiers = parse_answer(LLM_outputs)
