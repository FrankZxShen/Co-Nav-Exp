...
from src.SystemPrompt import (
    ...
    form_prompt_for_DecisionVLM_single, 
    form_prompt_for_DecisionVLM_Meta,
    ...
    while count_episodes < num_episodes:
        ...
        while not env.episode_over:
            if agent[0].l_step % args.num_local_steps == args.num_local_steps - 1 or agent[0].l_step == 0:
                ...
                while agent[0].l_step < 25:
                    ...
                    for i in range(num_agents):
                        ...
                        full_map1.append(torch.cat([fm.unsqueeze(0) for fm in full_map], dim=0))
                        full_map_pred1, _ = torch.max(full_map1[i], 0)
                        ...
                        # fn = 'Step-{}.png'.format(agent[0].l_step)
                        # cv2.imwrite(fn, full_rgb)  
                    ...
                    # logging.info(f"VLM_Decision_Meta_Pred: {Decision_Pred_Meta}")
                    ...
                    ##### VLM Process :>
                    Module_Prompt_Multi = ''
                    Module_Decision_Multi = ''
                    Module_Decision_Prompt_Multi = ''
                    ...
                    for i in range(num_agents):
                        ...
                        sem_map = Decision_Generation_Vis(args, agent[0].episode_n, agent[0].l_step, pose_pred, full_map_pred1, 
                                agent[i].goal_id, visited_vis, target_edge_map, Frontiers_dict, goal_points)
                        full_rgb = np.hstack((rgb, sem_map))
                        agent_objs[f"agent_{i}"] = yolo.run(rgb) # 记录一个时间步内每个智能体的目标检测信息
                        
                        # logging.info(agent_objs)

                        Module_Prompt = form_prompt_for_Module_Decision(goal_name, Decision_Pred_Meta, agent_objs[f"agent_{i}"])
                        # logging.info(f"Module_Prompt: {Module_Prompt}")
                        if Module_Prompt:
                            _, VLM_Module_Decision_Prompt_single = cogvlm2.simple_image_chat(User_Prompt=Module_Prompt, 
                                                                                            return_string_probabilities=None, img=full_rgb)
                            # logging.info(f"VLM_Module_Decision_Prompt_single: {VLM_Module_Decision_Prompt_single}")
                            VLM_Decision_Prompt_single = form_prompt_for_DecisionVLM_single(goal_name, start, Frontiers_dict)
                            # logging.info(f"VLM_Decision_Prompt_single: {VLM_Decision_Prompt_single}")
                            _, Decision_Pred_Single1 = cogvlm2.COT2(User_Prompt1=Module_Prompt, 
                                                                    User_Prompt2=VLM_Decision_Prompt_single,
                                                                    cot_pred1=VLM_Module_Decision_Prompt_single,
                                                                    return_string_probabilities=None, img=full_rgb)
                        else:
                            VLM_Decision_Prompt_single = form_prompt_for_DecisionVLM_single(goal_name, start, Frontiers_dict)
                            _, Decision_Pred_Single1 = cogvlm2.simple_image_chat(User_Prompt=VLM_Decision_Prompt_single, 
                                                                                return_string_probabilities=None, img=full_rgb)
                            # logging.info(f"VLM_Decision_Pred: {Decision_Pred_Single1}")
                        
                        Meta_Score, Meta_Choice = cogvlm2.COT2(User_Prompt1=VLM_Decision_Prompt_single, 
                                                   User_Prompt2=form_prompt_for_DecisionVLM_Meta(),
                                                   cot_pred1=Decision_Pred_Single1,
                        ...
                        if Choice == 0:
                            goal_points[i] = [int(x) for x in Frontiers_dict['frontier_0'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                        elif Choice == 1:
                            try:
                                goal_points[i] = [int(x) for x in Frontiers_dict['frontier_1'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                            except:
                                goal_points[i] = [int(x) for x in Frontiers_dict['frontier_0'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                        elif Choice == 2:
                            try:
                                goal_points[i] = [int(x) for x in Frontiers_dict['frontier_2'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                            except:
                                goal_points[i] = [int(x) for x in Frontiers_dict['frontier_0'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                        elif Choice == 3:
                            try:
                                goal_points[i] = [int(x) for x in Frontiers_dict['frontier_3'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                            except:
                                goal_points[i] = [int(x) for x in Frontiers_dict['frontier_0'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                        # if Module_Prompt:
                        #     ### T1
                        #     Module_Prompt_Multi += Module_Prompt
                        #     Module_Prompt_Multi += '\n'
                        #     Module_Decision_Multi += VLM_Module_Decision_Prompt_single
                        #     Module_Decision_Multi += '\n'
                        # ### T2
                        # Module_Decision_Prompt_Multi += VLM_Decision_Prompt_single
                        # Module_Decision_Prompt_Multi += '\n'
                        # Decision_Pred_Multi += Decision_Pred_Single1
                        # Decision_Pred_Multi += '\n'

                    # logging.info(f"1: {Module_Prompt_Multi}")
                    # logging.info(f"1.1: {Module_Decision_Multi}")
                    # logging.info(f"2: {Module_Decision_Prompt_Multi}")

                    # logging.info(f"Decisions: {Decisions}")
                    logging.info(f"Scores: {Scores}")
                    logging.info(f"goal points: {goal_points}")
                ...
                for ags in range(360 // args.turn_angle): # 旋转
                    ...
                    for i in range(num_agents):
                        ...
                        VLM_Perception_Prompt = form_prompt_for_PerceptionVLM(goal_name, agent_objs[f'agent_{i}'], agents_seg_list)
            ...
            while agent[0].l_step < 25:
                ...
                sem_map = Decision_Generation_Vis(args, agent[0].episode_n, agent[0].l_step, pose_pred, full_map_pred, 
                                agent[0].goal_id, visited_vis, target_edge_map, Frontiers_dict, goal_points)
                if args.visualize or args.print_images: 
                    Visualize(args, agent[0].episode_n, agent[0].l_step, pose_pred, full_map_pred, 
                                agent[0].goal_id, visited_vis, target_edge_map, Frontiers_dict, goal_points)
...
def Decision_Generation_Vis(args, episode_n, l_step, pose_pred, full_map_pred, goal_name, visited_vis, map_edge, Frontiers_dict, goal_points):
    ...
    if Frontiers_dict:
        for keys, value in Frontiers_dict.items():
            ...
            if match:
                ...
                # print(f"Centroid: ({centroid_x}, {centroid_y})")
                # print(f"Number: {number}")
                cv2.circle(sem_map_vis, (centroid_x+14, centroid_y-84), 5, color_black, -1)
                label = f"{alpha[alpha0]}"
                alpha0 += 1
                cv2.putText(sem_map_vis, label, (centroid_x+14+5, centroid_y-84+5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_black, 1)
