...
def Decision_Generation_Vis(args, agent_j, episode_n, l_step, pose_pred, full_map_pred, goal_name,
            ...
            if agent[0].l_step % args.num_local_steps == args.num_local_steps - 1 or agent[0].l_step == 0:
                ...
                if len(full_target_point_map) > 0:
                    ...
                    for j in range(num_agents):
                        ...
                        sem_map = Decision_Generation_Vis(args, j, agent[0].episode_n, agent[0].l_step, pose_pred, agent_MapPred[j], 
                                        agent[j].goal_id, visited_vis[j], agent_TargetEdgeMap[j], history_nodes, full_Frontiers_dict, goal_points=[])
                        ...
                        if len(agent_TargetPointMap[j]) > 0:
                            ...
                            logging.info(f'===== Agent_{j} No Frontier, History Mode =====')
                            #### 修改为历史node
                    ...
                    logging.info(f'===== No Frontier, History Mode =====')
                    logging.info(f'=====> Agent_{j} state: Step: {agent[j].l_step}; Angle: {start_o}')
                    if len(history_nodes) > 0:
                        logging.info(f'=====> history_nodes: {history_nodes}')
                        logging.info(f'=====> history_score: {history_score}')
                    for j in range(num_agents):
                        ...
                        sem_map = Decision_Generation_Vis(args, j, agent[0].episode_n, agent[0].l_step, pose_pred, agent_MapPred[j], 
                                        agent[j].goal_id, visited_vis[j], agent_TargetEdgeMap[j], history_nodes, Frontiers_dict=None, goal_points=[])
                        full_rgb = np.hstack((rgb, sem_map))
