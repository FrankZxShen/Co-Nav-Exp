...
def Decision_Generation_Vis(args, episode_n, l_step, pose_pred, full_map_pred, goal_name,
                             visited_vis, map_edge, history_nodes, Frontiers_dict, goal_points):
...
######################################################################################################################
            ...
            if agent[0].l_step % args.num_local_steps == args.num_local_steps - 1 or agent[0].l_step == 0:
                ...
                if len(full_target_point_map) > 0:
                    ...
                    for j in range(num_agents):
                        ...
                        # sem_map = Decision_Generation_Vis(args, agent[0].episode_n, agent[0].l_step, pose_pred, agent_MapPred[j], 
                        #                 agent[j].goal_id, visited_vis[j], agent_TargetEdgeMap[j], full_Frontiers_dict, goal_points)
                        full_rgb = np.hstack((rgb, full_sem_map))
                        ...
                        logging.info(f"Agent_{i}--VLM_PerceptionPR: {Perception_PR}")
                        ...
                        if len(agent_TargetPointMap[j]) > 0:
                            ...
                            if len(history_nodes) > 0:
                                ...
                                logging.info(f"Agent_{i}--FN_PR: {FN_PR}")
                                angle_score = Perception_PR[0] * FN_PR[0]
                                c_angle = int(start_o % 360)

                                if i == len(history_nodes) - 2 or len(history_nodes) == 1:
                            ...
                            if FN_PR[0] >= 0.50:
                                Meta_Prompt = form_prompt_for_DecisionVLM_Frontier(goal_name, start, full_Frontiers_dict)
                                ...
                                Meta_Prompt = form_prompt_for_DecisionVLM_History(goal_name, start, history_score, history_nodes)
                                Meta_Score, Meta_Choice = cogvlm2.COT3(User_Prompt1=VLM_Perception_Prompt, 
                                                            User_Prompt2=FN_Prompt,
                                                            User_Prompt3=Meta_Prompt,
                                                            cot_pred1=Perception_Pred,
                                                            cot_pred2=FN_Decision,
                                                            return_string_probabilities="[A, B, C, D, E]", img=full_rgb)
                                # Decisions.append(Meta_Choice)
                                Final_PR = Perception_weight_decision26(Meta_Score, Meta_Choice)

                            logging.info(f"Agent_{i}--Final_PR: {Final_PR}")
                            ...
                            if Choice == 0:
                                ...
                                try:
                                    goal_points[j] = [int(x) for x in full_Frontiers_dict['frontier_1'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                                except:
                                    goal_points[j] = [int(x) for x in full_Frontiers_dict['frontier_0'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                            elif Choice == 2:
                                try:
                                    goal_points[j] = [int(x) for x in full_Frontiers_dict['frontier_2'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                                except:
                                    goal_points[j] = [int(x) for x in full_Frontiers_dict['frontier_0'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                            elif Choice == 3:
                                try:
                                    goal_points[j] = [int(x) for x in full_Frontiers_dict['frontier_3'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
                                except:
                                    goal_points[j] = [int(x) for x in full_Frontiers_dict['frontier_0'].split('centroid: ')[1].split(', number: ')[0][1:-1].split(', ')]
            ...
            if args.visualize or args.print_images: 
                if num_agents == 1:
                    ...
                    height, _ = full_rgb.shape[:2]
                    legend_resized = cv2.resize(Legend, (Legend.shape[1], height))
                    img_show = np.hstack((full_rgb, legend_resized))
